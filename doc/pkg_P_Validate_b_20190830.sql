      ...
      BEGIN
         ...
         EXCEPTION
            ...
            WHEN OTHERS THEN
               ...
               v_message  := '单位编码:'||prm_aab001||'没有获取到单位基本信息'||SQLERRM;
               GOTO label_ERROR;
            END;
      END;
...
ELSE   
   ...
   IF     prm_iaa100>201812 AND  SYSDATE >dat_AAE030  THEN 
     ...
     END IF;
   END IF;
    IF  var_aab019='60' and (v_aae110 = '1' AND REC_TMP_PERSON.aae110 = '1') THEN
        SELECT ROUND(pkg_common.fun_p_getcontributionbase(null,prm_aab001,num_aac040,'0','01','1','1',prm_iaa100,prm_yab139))
                INTO num_yac004
                FROM  dual ;
        UPDATE IRAC01A2
           SET  yaa333 = ROUND(num_yac004)
         WHERE aac002 = REC_TMP_PERSON.aac002
           AND aac003 = REC_TMP_PERSON.aac003
           AND iaz018 = prm_iaz018;
   END IF;
...
PROCEDURE prc_p_checkInfoByYear(prm_aab001 IN VARCHAR2,
  ...
  IS
    ...
    num_aab156_04  xasi2.ab08.aab156%TYPE;
    num_aab156_05  xasi2.ab08.aab156%TYPE;
    num_aab240_02  xasi2.ab17.aab240%TYPE;
    num_aab240_03  xasi2.ab17.aab240%TYPE;
    num_aab240_04  xasi2.ab17.aab240%TYPE;
    num_aab240_05  xasi2.ab17.aab240%TYPE;
...
--          END IF;
       ...
       -- 工伤H01欠费金额
        ...
        where yae517  in( 'H01','H38')
         ...
         select aab240
          ...
          where aab001 = prm_aab001
            and aae140 = '03';    
        prm_msg :='单位存在欠费: : 失业: '||num_aab156_02||'元, 医疗: '||num_aab156_03||'元, 工伤: '||num_aab156_04||'元, 生育: '||num_aab156_05||'元; 待转金余额: 失业: '||num_aab240_02||'元, 医疗: '||num_aab240_03||'元, 工伤: '||num_aab240_04||'元, 生育: '||num_aab240_05
        ||'元, 欠费金额为各险种的欠费减去待转余额。单位请及时至税务部门缴费, 实收到账后方可进行年审。';
        prm_disabledBtn :='exportBtn,importBtn,retainBtn,applyBtn,printBtn1,printBtn2,printBtn3,printBtn4,printBtn5,viewBtn,cancelBtn';
        GOTO leb_over;
       END IF;
    ...
    IF num_count > 0 THEN
     ...
     prm_msg :='单位有未提交的人员增减信息，请撤销增减信息后重新进行年审操作！';
     prm_disabledBtn :='exportBtn,importBtn,retainBtn,applyBtn,cancelBtn,printBtn1,printBtn2,printBtn3,printBtn4,printBtn5,viewBtn';
     GOTO leb_over;
     END IF;
     --检查是否有未提交的养老未转入备案
    ...
    --modify by wanghm 20190817 个体工商 失业工伤山下线 2019年后年审 start
   SELECT count(1)
     INTO num_count
     FROM xasi2.ab01
    ...
    AND aab001 = prm_aab001;
   IF num_count <> 1 THEN
      prm_msg  := '单位编码:'||prm_aab001||'没有获取到单位类型信息'||SQLERRM;
      prm_disabledBtn :='exportBtn,importBtn,retainBtn,applyBtn,cancelBtn,printBtn1,printBtn2,printBtn3,printBtn4,printBtn5,viewBtn';
      GOTO leb_over;
   END IF;
   ...
   SELECT aab019
    ...
    IF num_aae001 >= 2019 AND var_aab019 ='60' THEN 
      SELECT to_char(PKG_COMMON.fun_p_getcontributionbase(null,prm_aab001,1,'0','02','1','1',num_aae002,prm_yab139))
        INTO prm_dxby60
        FROM dual;
      SELECT to_char(PKG_COMMON.fun_p_getcontributionbase(null,prm_aab001,20000000,'0','02','1','1',num_aae002,prm_yab139))
        INTO prm_gxby60
        FROM dual;
    END IF;
     --modify by wanghm 20190817 个体工商 失业工伤上下线 2019年后年审 end
    <<leb_over>>
    num_count :=0;
