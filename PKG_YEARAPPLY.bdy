     ...
     --获取单位参保信息
      ...
      CURSOR cur_ab05a1 IS
        SELECT aac001,
               ...
               NVL(yaa333,0) AS yaa333,--变更后医疗生育缴费基数
               NVL(yac005,0) AS yac005,--工伤缴费工资
               NVL(yaa444,0) AS yaa444--变更后工伤失业缴费基数
          FROM xasi2.ac01k8
         WHERE aab001 = prm_aab001
      ...
      --医疗四险的补差校验
          FOR rec_ab02 IN cur_ab02 LOOP
              ...
              IF num_aae041_year <= num_yae097 THEN
                      --获取人员补差信息  
                      IF rec_ab05a1.yac508 > 0 or  rec_ab05a1.yaa444 > 0 THEN   --如果原缴费基数大于0则说明这个人参加医疗四险
                   ...
                   and a.aac001 = v_aac001;
                    --提前结算的新旧缴费工资和基数写成一样的
                     INSERT INTO xasi2.ac01k8 (
                                            ...
                                            n_yac507,
                                            n_yac508,
                                            case when v_aae110 is not null then '21' ELSE v_aae110 end ,
                                            case when v_aae210 is not null then '21' ELSE v_aae210 end ,
                                            case when v_aae310 is not null then '21' ELSE v_aae310 end ,
                                            case when v_aae410 is not null then '21' ELSE v_aae410 end ,
                                            case when v_aae510 is not null then '21' ELSE v_aae510 end ,
                                            case when v_aae311 is not null then '21' ELSE v_aae311 end ,
                                            prm_aae001,
                                            v_yac503,
                                            n_yac005,
                                            prm_yae092,
   ...
   BEGIN
    ...
    END IF;
     ...
     UPDATE wsjb.irac08a1
      ...
      INSERT INTO xasi2.ab05a2 (aab001,
                                  aae001,
                                  yab007,
                                  aae011)
                          VALUES (prm_aab001,
...
PROCEDURE prc_insertIRAC08A1(prm_aab001       IN     irab01.aab001%TYPE,--单位编号  必填
   ...
   BEGIN
     ...
     -- 201905 起 养老单位费率改为16%  modify by wanghm 20190830 start
      ...
      INSERT INTO wsjb.irac08a1 (YAE202,             --明细流水号,VARCHAR2
                   ...
                   VALUES (xasi2.pkg_comm.FUN_GETSEQUENCE(NULL,'YAE202'),
                           ...
                           0,
                           0,
                           CASE WHEN var_aab019 = '60' THEN round(prm_aac040*0.12,2) ELSE round(prm_aac040*0.16,2) END,
                           0,
                           prm_yae518,
                           prm_aae076,
                           prm_aae011,
       ...
       END IF; 
     -- 201905 起 养老单位费率改为26%  modify by wanghm 20190830 end
...
END prc_insertIRAC08A1;
   ...
   *****************************************************************************/
   ...
   BEGIN
      ...
      --不再直接补差 改为创建定时任务  20190830 by wanghm start
      /*  
       ...
       */
       ...
       IF prm_AppCode <> gn_def_OK THEN
           ROLLBACK;
           prm_AppCode  :=  gn_def_ERR;
           prm_ErrorMsg := '创建定时任务pkg_p_createJob出错:'||prm_ErrorMsg;
           RETURN;
       END IF;
      --不再直接补差 改为创建定时任务  20190830 by wanghm end
        ...
        if num_count>0 then
            ...
            where aab001 = prm_aab001
             ...
             select (trunc((v_aab121_new / v_aab121_old), 4) - 1) * 100
              ...
              if v_proportions <= v_proportions_constant then
                 v_proportions_msg:='01险种'||v_minaae041||'基数降低比例过高'||v_proportions;
                 GOTO leb_next;
              end if;
          --非单养老单位处理 4险
          for ab02_rec in ab02_cur loop
            ...
            select (trunc((v_aab121_new / v_aab121_old), 4) - 1) * 100
              ...
              from DUAL;
              if v_proportions <= v_proportions_constant then
                 v_proportions_msg:=ab02_rec.aae140||'险种'||v_minaae041||'基数降低比例过高'||v_proportions;
                 GOTO leb_next;
              end if;
          end loop;
        else    --单养老
...
END prc_YearApplyJSProportions;
 ...
 PROCEDURE prc_YearSalaryBCJOB(
                           ...
                           prm_yab019        IN     VARCHAR2  )           --类型标志
 IS                   
            prm_AppCode           VARCHAR2(20);       --执行代码
            prm_ErrorMsg            VARCHAR2(1000);     --执行结果 
 BEGIN
  ...
  prc_YearSalaryBC(
      ...
      IF prm_AppCode <> gn_def_OK THEN
       ROLLBACK;
        prm_AppCode  :=  gn_def_ERR;
        prm_ErrorMsg := '调用核定过程prc_YearSalaryBC出错:'||prm_ErrorMsg;
        xasi2.pkg_p_yearOfCarefulFinal.insertQRTZ_JOB_MSGS(prm_jobid,prm_aab001,prm_aae011,null,sysdate,'1',prm_ErrorMsg,prm_ErrorMsg);
       RETURN;
       ELSE
        xasi2.pkg_p_yearOfCarefulFinal.insertQRTZ_JOB_MSGS(prm_jobid,prm_aab001,prm_aae011,null,sysdate,'1',prm_ErrorMsg,prm_ErrorMsg);
       END IF;
       
   END prc_YearSalaryBCJOB;
    ...
    IS
      ...
      var_yab222       VARCHAR2(30);
      var_week          VARCHAR2(20);
      NUM_AB08_PERNUM NUMBER;
      NUM_COUNT       NUMBER;
      NUM_COUNT1      NUMBER;
      num_count_aa02  NUMBER;
      var_ywlx           VARCHAR2(20);
      var_daytype     VARCHAR2(20);
    ...
    BEGIN
      ...
      IF prm_aab001 IS NULL THEN
         prm_AppCode := gn_def_ERR;
         prm_ErrMsg := '日常年审创建定时任务出错：单位编号不能为空!';
         RETURN;
      END IF;
      
      IF prm_aae001 IS NULL THEN
         prm_AppCode := gn_def_ERR;
         prm_ErrMsg := '日常年审创建定时任务出错：年审年度不能为空!';
         RETURN;
      END IF;
      
      IF prm_yab139 IS NULL THEN
         prm_AppCode := gn_def_ERR;
         prm_ErrMsg := '日常年审创建定时任务出错：经办分中心不能为空!';
         RETURN;
      END IF;
      
      IF prm_aae001 < 2019 THEN
         prm_AppCode := gn_def_ERR;
         prm_ErrMsg := '日常年审创建定时任务出错：年审年度不能小于2019!';
         RETURN;
      END IF;
      
      var_yab222 := xasi2.pkg_comm.fun_GetSequence(NULL,'yab222');
      num_jobid := xasi2.pkg_comm.fun_GetSequence(NULL,'JOBID');
      var_jobname := prm_aab001||'单位日常年审';
      var_interval := '';   
      var_what := 'wsjb.PKG_YearApply.prc_YearSalaryBCJOB('''|| num_jobid  ||''','''
                                                                          ||  prm_aab001||''','''
                                                                          ||  prm_aae001||''','''
                                                                          || prm_aae011 ||''','''
                                                                          || prm_yab139 ||''','''
      ...
      IF NUM_COUNT > 0 THEN
         prm_AppCode := gn_def_ERR;
         prm_ErrMsg := '日常年审创建定时任务出错：经办分中心'||prm_yab139||'已经存在待执行的定时任务，请勿重复提交!';
         RETURN;
      END IF;
      ...
      SELECT  COUNT(1)
        INTO  NUM_COUNT1
        FROM  xasi2.ac01k8 a
       WHERE  a.aab001 = prm_aab001  -- 单位id -->
         AND  a.aae001 = prm_aae001;  -- 年度  -->       
         
      --待处理人数大于2000人，晚上处理
      IF NUM_COUNT1 > 2000 THEN
         var_daytype :=  '2';
      END IF;
      ...
      SELECT COUNT(1)
        INTO num_count_aa02
        FROM xasi2.aa02 a
       WHERE a.aae001 = prm_aae001   -- 年度 -->
         AND a.yaa001 = '16';
      IF num_count_aa02 > 0 THEN
          --获取定时任务开始时间
          xasi2.pkg_p_yearOfCarefulFinal.prc_p_getJobStartTime (var_ywlx,    --业务类型
                                 var_daytype, --执行时间类型
                                 var_next_date, --开始时间    
                                 prm_AppCode,      --错误代码
                                 prm_ErrMsg);
          IF prm_AppCode <> gn_def_OK THEN
             prm_AppCode := gn_def_ERR;
             prm_ErrMsg := '日常年审创建定时任务，获取定时任务开始时间出错：,'||prm_ErrMsg||SQLERRM||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE();
             RETURN;
          END IF;
           
          xasi2.pkg_YHCIP.prc_oracleJob(num_jobid,    
                                  var_jobname,
                                  var_what,
                                  'to_date('''||var_next_date||''',''yyyy-MM-dd HH24:mi:ss'')',
                                  var_interval,
                                  prm_aae011,
                                  prm_AppCode,
                                  prm_ErrMsg
          ...
          IF prm_AppCode <> gn_def_OK THEN
             prm_AppCode := gn_def_ERR;
             prm_ErrMsg := '日常年审创建定时任务出错：'||prm_ErrMsg||','||SQLERRM||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE();
             RETURN;
          ELSE
             prm_AppCode := gn_def_OK;
             prm_ErrMsg := '';
    ...
    EXCEPTION
      WHEN OTHERS THEN
         prm_AppCode  :=  gn_def_ERR;
         prm_ErrMsg   := '年审创建定时任务出错，错误原因为：'||prm_ErrMsg||','||SQLERRM||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE();
         RETURN;
    END pkg_p_createJob;
